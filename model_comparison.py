...
def run_model_comparison(csv_path='output/cleaned_dataset.csv', chart_dir='output/charts'):
    ...
    # Step 2: Prepare features and one-hot encode
    ...
    X_encoded = pd.get_dummies(df[features].copy(), drop_first=True)
    sub_data_fractions = [0.05, 0.10, 0.20]
    all_results  = []
    best_record  = {'f1': -1, 'report': '', 'label': ''}

    # Step 3: Train and evaluate each model across 5%, 10%, 20% sub-samples — Fig. 8
    for frac in sub_data_fractions:
        label = f"{int(frac * 100)}%"
        print(f"\n{'='*60}")
        print(f"[*] SUB-DATA: {label}")
        print('='*60)

        df_sub = (df.groupby('Severity_Class', group_keys=False)
        ...
        for model_name, model in get_models().items():
            ...
            try:
                ...
                # Class 3 Recall separately — key metric in Table 1
                c3_rec = recall_score((y_test==3).astype(int), (y_pred==3).astype(int), zero_division=0) * 100

                print(f"    ✓ Acc={acc:.2f}% | F1={f1:.2f}% | C3 Recall={c3_rec:.2f}% | Time={train_time:.1f}s")
    ...
    print("="*95)
    print(f"\n[★] Best model: {best_record['label']}")
    print(best_record['report'])

    # Step 5: F1-Score per sub-data — Fig. 8 left panels
    ...
    # Step 6: Average 4 metrics across sub-datasets — Fig. 8 right panel
    ...
    for p in ax.patches:
        ...
        if h > 0:
            ax.annotate(f'{h:.1f}', (p.get_x() + p.get_width() / 2., h),
                        ha='center', va='bottom', xytext=(0, 4), textcoords='offset points', fontsize=8)
    plt.tight_layout()
    ...
    # Step 7: F1-Score heatmap (Model × Sub-Data)
    ...
    for bar, val in zip(bars, avg_time.values):
        plt.text(val + 0.1, bar.get_y() + bar.get_height() / 2, f'{val:.1f}s', va='center', fontsize=10)
    ...
    print("\n[V] All done! Charts saved to:", chart_dir)
