...
def run_ml_pipeline(csv_path='output/cleaned_dataset.csv', chart_dir='output/charts'):
    ...
    # Step 4: Train/test split and train Random Forest with cost-sensitive weights (1:3:10)
    ...
    # Step 5: Evaluate symmetric baseline (tau=0.5) — Table 2
    y_pred = rf_model.predict(X_test)
    acc = accuracy_score(y_test, y_pred)
    print(f"\n[+] ACCURACY: {acc*100:.2f}%")
    print("\n--- CLASSIFICATION REPORT (symmetric baseline) ---")
    target_names = ['Low Risk', 'Moderate Risk', 'High Risk']
    print(classification_report(y_test, y_pred, target_names=target_names))

    # Step 6: Confusion matrix — Fig. 9
    cm = confusion_matrix(y_test, y_pred)
    plt.figure(figsize=(8, 6))
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
                xticklabels=target_names, yticklabels=target_names)
    plt.title('Confusion Matrix: Severity Prediction — Symmetric Baseline', fontsize=12)
    plt.ylabel('Actual')
    plt.xlabel('Predicted')
    plt.tight_layout()
    plt.savefig(os.path.join(chart_dir, 'Confusion_Matrix_Baseline.png'), dpi=300)
    plt.close()

    # Step 7: Feature importance — Fig. 10
    importances = rf_model.feature_importances_
    feat_df = pd.DataFrame({'Feature': X_encoded.columns, 'Importance': importances})
    feat_df = feat_df.sort_values(by='Importance', ascending=False).head(10)
    feat_df = feat_df[~feat_df['Feature'].str.contains('UNKNOWN', case=False, na=False)]
    ...
    # Step 8: Save trained model and column schema
    joblib.dump(rf_model, 'output/rf_model_m1.pkl')
    joblib.dump(list(X_encoded.columns), 'output/model_columns_m1.pkl')

    print(f"\n[V] Completed! Model saved at: output/rf_model_m1.pkl")
